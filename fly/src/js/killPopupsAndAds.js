export default function () {
  ...
  if (DEBUG2) console.log("key", "flySwatterTech-" + host);
  let donotuse = window.localStorage["flySwatterTech-" + host];
  ...
  ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  // If DISABLED, code below will NOT run!
  ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  if (IS_GOOGLE && window.location.pathname.includes("/search")) {
    ...
    setTimeout(removeAnnoyingAnnoyances.bind({ timeout: 500 }), 500);
    setTimeout(removeAnnoyingAnnoyances.bind({ timeout: 1500 }), 1500);
    setTimeout(removeAnnoyingAnnoyances.bind({ timeout: 5000 }), 5000);
    setTimeout(removeAnnoyingAnnoyances.bind({ timeout: 10000 }), 10000);
  }
  ...
  function hideAA(el, because = "", force = false) {
    ...
    if (DEBUG1)
      console.error(
        "HIDE " + el._css.position,
        because ? "because " + because : "",
        [el.id, el.className, el.style.display, el.innerText]
      );
  ...
  function removeAnnoyingAnnoyances() {
    ...
    for_each_element: for (let i = 0; i < len; i++) {
      ...
      if (el._css.position === "fixed" || el._css.position === "sticky") {
        ...
        el._height =
          (el._css.height ? Number(el._css.height.replace(/[^0-9]/g, "")) : 0) +
          el._paddingTop;
        el._width =
          (el._css.width ? Number(el._css.width.replace(/[^0-9]/g, "")) : 0) +
          el._paddingSides;
        el._bottom =
          Number(
            (el._css.getPropertyValue("bottom") || "").replace(/[^\d.]+/g, "")
          ) + Number((el._css.marginBottom || "").replace(/[^\d.]+/g, ""));
        el._right =
          Number(
            (el._css.getPropertyValue("right") || "").replace(/[^\d.]+/g, "")
          ) + Number((el._css.marginRight || "").replace(/[^\d.]+/g, ""));
        el._top =
          Number(
            (el._css.getPropertyValue("top") || "").replace(/[^\d.]+/g, "")
         ...
         */
        el._is_cookies = (el._innerText + el._class + el._id).includes(
          "cookie"
        );
        ...
        if (
          el._innerText.includes("email") ||
          el._innerText.includes("signup")
        ) {
        ...
        if (
          el._bottom === 0 &&
          el._width === window.width &&
          el._innerText.length < 20
        ) {
          el._is_ad = true;
        }
        ...
        for (let elb of el._iframes) {
          let elb_height = el._css.height
            ? Number(el._css.height.replace(/[^0-9]/g, ""))
            : 0;
          if (elb_height < 300) {
        ...
        if (el._is_cookies) {
          if (DEBUG3)
            console.log("el._is_cookies buttons", el._buttons_and_links);
        }
        let bi = -1;
        for (let elb of el._buttons_and_links.reverse()) {
          ...
          let elb_innerText = (elb.innerText || elb.value || "")
            ...
            .toLowerCase()
            .replace(/[^\w×]+/g, "");
          let elb_innerHTML = (elb.innerHTML || "")
            .substring(0, 300)
            .toLowerCase();
          let elb_texts =
            elb_innerText + " " + elb_class + " " + elb_id + " " + elb_title;
          if (DEBUG3) console.log("--- --- " + bi + " button text", elb_texts);
          if (DEBUG4)
            console.log("--- --- " + bi + " button HTML", elb_innerHTML);
          ...
          if (el._is_cookies) {
            if (click_yes.includes(elb_innerText)) {
              if (DEBUG3)
                console.error(
                  "--- clicked exact [click_yes] COOKIES",
                  elb_texts
                );
              elb.click();
              continue;
          ...
          if (click_no.includes(elb_innerText)) {
            if (DEBUG3)
              console.error("--- clicked exact [click_no]", elb_texts);
            elb.click();
            continue;
          }
          ...
          if (el._is_cookies || el._is_newsletter || el._is_ad) {
            if (elb_texts.includes("agree")) {
              if (DEBUG3)
                console.error(
                  '--- clicked includes "agree / nothanks" COOKIES',
                  elb_texts
                );
              elb.click();
              continue;
            ...
            if (
              ...
              if (DEBUG3)
                console.error('--- clicked text === "x / close"', elb_texts);
              elb.click();
              continue;
            }
            ...
            if (elb_texts.includes("dismiss") || elb_texts.includes("close")) {
              ...
              if (DEBUG3)
                console.error(
                  '--- clicked texts includes "dismiss / close"',
                  elb_texts
                );
              elb.click();
              continue;
          ...
          if (click_always.includes(elb_innerText)) {
            if (DEBUG3)
              console.error(
                "--- clicked exact [click_always] COOKIES",
                elb_texts
              );
            elb.click();
            continue;
  ...
  function fixGoogleAds() {
    let num_search_results = Array.from(
      window.document.body.querySelectorAll("#search h3")
    ).length;
    ...
    let adlists = window.document.querySelectorAll(
      '#taw [role="list"], #taw [role="list"]'
    );
    ...
    if (topStoriesTitle) {
      try {
        ...
        topStoriesTitle.parentElement.parentElement.parentElement.style.display =
          "none";
      } catch (e) {}
    }
    ...
    let expandables = window.document.querySelectorAll(
    ...
    }
    {
      ...
      if (found) {
        found.parentElement.parentElement.parentElement.parentElement.style.display =
          "none";
      }
    }
